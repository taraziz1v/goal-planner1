...
export const InitialState: React.FC<InitialStateProps> = ({ onGenerate, isLoading }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 animate-fadeIn">
        <div className="relative flex items-center justify-center h-28 w-28 rounded-full bg-slate-900 border border-white/10 shadow-2xl mb-8">
          {/* Animated Glow Rings */}
          <div className="absolute inset-0 rounded-full border border-brand-primary/30 animate-ping opacity-75" />
          <div className="absolute -inset-4 rounded-full bg-gradient-to-tr from-brand-primary/10 to-brand-secondary/10 blur-xl animate-pulse" />
          ...
          {LOADING_STEPS.map((step, idx) => (
            <div 
              ...
              className={`flex items-center gap-2 transition-all duration-300 ${
                idx === loadingStep 
                  ? 'text-brand-secondary translate-x-1 font-semibold' 
                  : idx < loadingStep 
                    ? 'text-slate-500' 
                    : 'text-slate-800'
              }`}
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 sm:py-16 animate-fadeIn">
      ...
      {/* Decorative Blob */}
      <div className="absolute top-20 left-1/2 -translate-x-1/2 -z-10 h-72 w-72 rounded-full bg-gradient-to-tr from-brand-primary/10 to-brand-secondary/10 blur-3xl opacity-60 animate-pulse-slow" />
      
      {/* Hero Header */}
      <div className="text-center space-y-4 mb-10">
        <div className="inline-flex items-center gap-2 rounded-full bg-white/5 border border-white/10 px-4 py-1.5 text-xs text-brand-primary font-bold select-none shadow-sm">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-2.5">
            {PRESETS.map((preset, idx) => (
              ...
              >
                <span className="text-xs font-bold text-slate-300 group-hover:text-brand-primary transition-colors">
                  {preset.label}
                </span>
          ...
          <div className="rounded-2xl border border-white/8 bg-slate-900/30 backdrop-blur-md p-6 sm:p-8 shadow-2xl relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3.5 px-5 rounded-xl text-sm font-bold text-slate-950 shadow-xl shadow-brand-primary/10 bg-gradient-to-r from-brand-primary to-brand-secondary hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-300 group active:scale-[0.98]"
              >
                Generate Roadmap
